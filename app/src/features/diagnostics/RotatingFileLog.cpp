...
const char* severity_name(RotatingFileLog::Severity severity) {
    switch (severity) {
    case RotatingFileLog::Severity::Info:
        return "INFO";
    case RotatingFileLog::Severity::Warning:
        return "WARN";
    case RotatingFileLog::Severity::Error:
        return "ERROR";
    }
    return "INFO";
}
...
std::string RotatingFileLog::sanitize(std::string_view message) {
    std::string result;
    std::size_t start = 0;
    while (start <= message.size()) {
        const auto end = message.find('\n', start);
        const auto lineEnd =
            end == std::string_view::npos ? message.size() : end;
...
bool RotatingFileLog::append(const std::string& path,
